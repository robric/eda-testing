...
https://docs.eda.dev/25.8/development/api/#__tabbed_1_2
First get a token via Keycloak authentication
...
curl -sk \
  -X GET "$EDA_API_URL/openapi/v3/apps/connect.eda.nokia.com/v1alpha1" \
  -H "Authorization: Bearer $EDA_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  | jq .
